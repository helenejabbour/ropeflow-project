"""
Build 05_Full_Pipeline_V05.ipynb
Run: python "src/Full pipeline/build_v05.py"
"""
import sys, json, os
...
# ── Load V04 to reuse its cells verbatim ──────────────────────
with open(os.path.join(HERE, '04_Full_Pipeline_V04.ipynb'), encoding='utf-8') as f:
    v04 = json.load(f)
...
def v04_code(idx):
    """Return the source of a V04 code cell as a string."""
    return ''.join(v04['cells'][idx]['source'])
...
CELL_TITLE = """\
# Rope Flow — Full Pipeline V05
## Heterogeneous Sessions + Train/Test Split + Test Error Evaluation

**Course:** MECH 798M / EECE 798K
**Pipeline:** V04 signal processing → multi-source session discovery
...
CELL_SETUP = v04_code(2)  # imports + CONFIG + KNOWN_PATTERNS + PAL

CELL_CORE_FUNCTIONS = v04_code(4)  # all signal-processing / feature-extraction functions
...
def discover_all_sessions(processed_dir):
    \"\"\"Combine homogeneous + heterogeneous entries.\"\"\"
    return discover_sessions(processed_dir) + discover_heterogeneous_sessions(processed_dir)
...
plt.tight_layout()
plt.savefig('fig_v05_confusion_matrices.png', bbox_inches='tight')
plt.show()
"""
...
plt.tight_layout()
plt.savefig('fig_v05_f1_comparison.png', bbox_inches='tight')
plt.show()
"""
...
plt.tight_layout()
plt.savefig('fig_v05_feature_importance.png', bbox_inches='tight')
plt.show()
"""
...
cells = [
    ...
    md("---\n## 2. Core pipeline functions (V02 — validated)\n"
       "All feature extraction, template, and base classification functions reused verbatim from V04."),  # 4
    ...
    md("---\n## 5. Stratified train / test split"),  # 10
    code(CELL_SPLIT),                            # 11
...
OUT = os.path.join(HERE, '05_Full_Pipeline_V05.ipynb')
