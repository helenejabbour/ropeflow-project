...
def filter_peaks_by_period(peaks, factor=0.5):
    """Remove peaks with physically implausible inter-peak intervals."""
    if len(peaks) < 3:
        return peaks
...
def detect_cycles(gyr_degs, fs):
    """
    Detect rope-flow cycles from angular velocity magnitude ||omega(t)||.
    Uses physical prominence threshold (CYCLE_PROMINENCE_DEGS) instead of
    bandpassed single-axis approach which retains harmonic sub-peaks.
    """
...
def save_plot(t, acc_g_raw, acc_g_filt, acc_linear,
              sig_bp, mag_smooth, peaks, dom, gaps, session_name, dev, gyr_filt):
